...
x <- tibble(p_IB = map_int(1:n, ~ roll_two_dice()),
     ...
     mutate(IB_craps = p_IB %in% c(7, 11),
            ...
            winner = case_when(
              IB_craps > JM_craps ~ "Ishan Wins",
              JM_craps > IB_craps ~ "John Mark Wins",
              TRUE ~ "Tie"),
            contest = rep(1:number_of_contests, each = contest_size)) %>% 
     group_by(contest) %>% 
     summarize(JM_wins = sum(winner == "John Mark Wins"),
               IB_wins = sum(winner == "Ishan Wins"),
               ties = sum(winner == "Tie"))

x

round(100*sum(x$JM_wins >= 4)/10000, 3)
```
